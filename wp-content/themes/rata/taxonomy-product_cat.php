...
if ($term && isset($term->term_id)) {
    ...
    // Custom query to filter by category and tag
    $args = [
        'post_type' => 'product',
        'posts_per_page' => 16,
        'paged' => $paged,
...
<main class="container mx-auto px-4 md:px-0">
	...
	<header class="w-full">
		...
		<div class="w-full h-[32px] md:h-[48px] border-b border-b-[#EAEAEA] flex">
			<p class="text-[#595959] leading-[32px] md:leading-[48px] translate-y-[10px] md:translate-y-[18px] opacity-0">Explore All</p><h1 class="translate-y-[2px] md:translate-y-[10px]"><?php echo esc_html($category_name); ?></h1>
		</div>
		<div class="w-full h-[32px] md:h-[48px] border-b border-b-[#EAEAEA] flex items-center justify-between">
			...
			<p class="text-gray-600 max-w-2xl mx-auto text-righr"><?php echo esc_html($category_description); ?></p>
		</div>
	</header>
	...
	<section class="py-12 w-full">
		<div class="flex w-full justify-between items-center mb-6">
			...
			<form method="get" class="flex items-center">
				<input type="hidden" name="tag" value="<?php echo esc_attr($current_tag); ?>">
			...
			<?php if ( $wp_query->max_num_pages > 1 ) : ?>
				<!-- Show Load More Button -->
				 <div class="flex justify-center mt-8">
					<button id="load-more" class="bg-golden-yellow px-6 py-3 shadow-lg">Load More <span class="arrow"><span class="line"></span></span></button>
				 </div>
			<?php endif; ?>
			<!-- <div class="mt-8">
		...
		<?php else: ?>
			<p class="text-center text-gray-500">No products found in this category<?php echo $current_tag ? ' and tag' : ''; ?>.</p>
		<?php endif; ?>
	</section>
</main>
